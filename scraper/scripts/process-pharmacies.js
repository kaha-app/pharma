 ...
 * Transforms raw Google Maps scraper CSV → Clean JSON matching target schema
 * Handles: CSV parsing, field mapping, deduplication, validation
 */
...
// Configuration
const CONFIG = {
  inputCsv: path.join(__dirname, '..', 'raw-results.csv'),
  outputJson: path.join(__dirname, '..', 'pharmacies.json'),
  areasFile: path.join(__dirname, '..', 'areas.json'),
};
 ...
 */
function main() {
  console.log('🏥 Pharmacy Data Processor\n');
  
  // Check if input file exists
  if (!fs.existsSync(CONFIG.inputCsv)) {
    console.error(`❌ Input file not found: ${CONFIG.inputCsv}`);
    console.log('\nRun the scraper first:');
    console.log('docker run --rm -v "${PWD}/scraper-test:/scraper-test" gosom/google-maps-scraper:latest-rod -input /scraper-test/input.txt -results /scraper-test/raw-results.csv -c 1 -depth 1 -lang en');
    process.exit(1);
  }
  ...
  const { unique, duplicateCount } = deduplicate(valid);
  ...
  console.log(`   Final pharmacies: ${finalOutput.length}`);
  
  // Show sample
  if (finalOutput.length > 0) {
    console.log('\n📋 Sample output (first pharmacy):');
    console.log(JSON.stringify(finalOutput[0], null, 2));
  }
