/**
 * Pharmacy Data Processor - Tier 3
 * Processes raw-results-tier3.csv → pharmacies.json
 * MERGES with existing Tier 1 + Tier 2 data
 */
...
// Configuration
const CONFIG = {
  inputCsv: path.join(__dirname, '..', 'raw-results-tier3.csv'),
  existingJson: path.join(__dirname, '..', 'pharmacies.json'),
  outputJson: path.join(__dirname, '..', 'pharmacies.json'),
  areasFile: path.join(__dirname, '..', 'areas-tier3.json'),
};
 ...
 */
function main() {
  console.log('🏥 Pharmacy Data Processor (Tier 3 - FINAL MERGE)\n');
  
  // Check if input file exists
  if (!fs.existsSync(CONFIG.inputCsv)) {
    console.error(`❌ Input file not found: ${CONFIG.inputCsv}`);
    console.log('\nRun the scraper first: ./run-tier3-scrape.ps1');
    process.exit(1);
  }
  
  // Load existing Tier 1 + Tier 2 data
  let existingPharmacies = [];
  if (fs.existsSync(CONFIG.existingJson)) {
    console.log('📂 Loading existing Tier 1 + Tier 2 data...');
  ...
  // Read and parse CSV
  console.log('📖 Reading Tier 3 CSV...');
  ...
  console.log('🔗 Merging with Tier 1 + Tier 2 data...');
  ...
  // Summary
  console.log('📊 Summary:');
  console.log(`   Tier 1 + Tier 2 pharmacies: ${existingPharmacies.length}`);
  console.log(`   Tier 3 raw records: ${rawRecords.length}`);
  console.log(`   Tier 3 after validation: ${valid.length}`);
