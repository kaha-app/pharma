/**
 * Pharmacy Data Processor - Depth 5 Version
 * Processes raw-results-depth5.csv → pharmacies.json
 */

const fs = require('fs');
const path = require('path');

// Configuration - UPDATED FOR DEPTH 5 FILE
const CONFIG = {
  inputCsv: path.join(__dirname, '..', 'raw-results-depth5.csv'),
 ...
 */
function main() {
  console.log('🏥 Pharmacy Data Processor (Depth 5)\n');
  
  // Check if input file exists
  if (!fs.existsSync(CONFIG.inputCsv)) {
    console.error(`❌ Input file not found: ${CONFIG.inputCsv}`);
    console.log('\nRun the scraper first: ./run-full-scrape-depth5.ps1');
    process.exit(1);
  }
